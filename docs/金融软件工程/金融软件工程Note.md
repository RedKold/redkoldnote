...
- 何为模块间的**耦合度**? 
	...
	- 耦合类型
		...
		- **特征耦合**：**（标记耦合）一组模块通过参数表**传递记录信息
		- **控制耦合**：模块通过传递开关、标志、名字等控制信息，明显地控制、选择另一个模块的功能
		- **外部耦合**：**允许一组模块访问同一个全局变量**。依赖共享的外部资源
		- **公共耦合**：允许一组模块访问同一个全局性的数据结构。（很强的连接模式！）
		- **内容耦合**：一组模块可以直接调用另一个模块中的数据，或者允许一个模块直接转移到另一个模块中。（严格禁止）
		- **时间耦合**（依赖执行顺序）
	...
	- **基于容器和镜像的部署**
		- **什么是容器**
			- 容器是一个视图隔离、资源可限制、具有独立文件系统的进程集合
				...
				- 容器具有独立文件系统
		- **容器镜像**
			- **容器运行时所需要的所有文件集合**（rootfs）
				- 打包应用代码+应用运行所需要的所有依赖文件
			- ![image.png|500](https://kold.oss-cn-shanghai.aliyuncs.com/20250612195838.png)
	
		- **容器部署流程**
			- 选定基础镜像
			- 编写 Dockfile 脚本
			- 根据 Dockfile 脚本创建软件镜像
			- 基于软件镜像创建并运行 docker 容器
			- 这使你想到了  [docker]([Docker 教程 | 菜鸟教程](https://www.runoob.com/docker/docker-tutorial.html))
---
- **基于部署图来搭建环境和部署**
	- **设计阶段的部署图**
		- 计算节点
...
- 软件部署的 CASE 工具
	- Fat jar
		- Java 程序的打包部署和安装
	- Installer Projects
		- 用于创建 Microsoft Windows 应用安装程序和安装包
		- 作为 visual studio 的拓展
	- Jenkins
		- 针对 Java 应用，实现持续集成和部署的软件工具


![image.png|600](https://kold.oss-cn-shanghai.aliyuncs.com/20250612203340.png)
